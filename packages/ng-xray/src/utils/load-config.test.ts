...
describe("loadConfig", () => {
  ...
  it("returns null and warns on malformed JSON config", () => {
    ...
    expect(result).toBeNull();
    expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining("could not be parsed"));
  });
  ...
  it("normalizes invalid architecture config fields from package.json", () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining("invalid architecture config"));
  });
});
