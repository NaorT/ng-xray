...
const RULE_TEMPLATES: Record<string, PromptTemplate> = {
  ...
  'bypass-security-trust': {
    riskLevel: 'high',
    whyItMatters: 'bypassSecurityTrust* disables Angular\'s built-in XSS sanitization. If the input comes from user data, API responses, or URL parameters, an attacker can inject malicious scripts that steal cookies, session tokens, or perform actions on behalf of the user.',
    investigationSteps: [
  ...
  'eval-usage': {
    riskLevel: 'high',
    whyItMatters: 'eval() executes arbitrary strings as JavaScript code. If the string comes from user input, query parameters, or external data, an attacker can inject and execute arbitrary code in the user\'s browser.',
    investigationSteps: [
  ...
  'hardcoded-secret': {
    riskLevel: 'high',
    whyItMatters: 'Hardcoded secrets in source code are committed to version control history permanently. Even if deleted later, they remain in git history. Anyone with repository access (including CI systems, contractors, and leaked repos) can extract them.',
    investigationSteps: [
  ...
  'innerhtml-binding': {
    riskLevel: 'medium',
    whyItMatters: 'While Angular sanitizes [innerHTML] by default, complex HTML from APIs or user input can bypass sanitization in edge cases. Direct innerHTML usage also makes the template harder to reason about security-wise.',
    investigationSteps: [
...
const DEFAULT_TEMPLATE: PromptTemplate = {
  riskLevel: 'medium',
  whyItMatters: 'This issue was flagged by ng-xray as a code quality concern that should be addressed to improve the overall health of the codebase.',
  investigationSteps: [
...
export const generateCursorPrompt = (diagnostic: Diagnostic): string => {
  ...
  const steps = template.investigationSteps
    .map((step, i) => `${i + 1}. ${step}`)
    .join('\n');
