...
export const RULE_DOCS: Record<string, RuleDoc> = {
  'missing-onpush': {
    ...
    whyItMatters: 'Without OnPush, Angular runs change detection on every browser event for this component. This can cause hundreds of unnecessary re-checks per second in large applications.',
  ...
  'missing-trackby': {
    ...
    category: 'performance',
    whyItMatters: 'Without trackBy, Angular destroys and recreates DOM elements on every change. With large lists this causes jank and memory thrashing.',
  ...
  'non-standalone-component': {
    ...
    whyItMatters: 'NgModules add indirection and increase bundle complexity. Standalone components are the recommended pattern since Angular 15+ and mandatory for best tree-shaking.',
  ...
  'dead-code-generic': {
    ...
    category: 'dead-code',
    whyItMatters: 'Dead exports increase bundle size and cognitive load. They make it harder to understand what code is actually used.',
  ...
  'unused-angular-entity': {
    ...
    category: 'dead-code',
    whyItMatters: 'Unused components, services, pipes, or directives add dead weight to the bundle and confuse developers navigating the codebase.',
  ...
  'unused-class-member': {
    ...
    category: 'dead-code',
    whyItMatters: 'Unused properties and methods add noise to classes and can mask the component\'s true interface. They are also potential maintenance hazards.',
  ...
  'circular-dependency': {
    ...
    whyItMatters: 'Circular imports can cause undefined values at runtime, make the dependency graph fragile, and prevent effective tree-shaking.',
  ...
  'deep-import': {
    ...
    category: 'architecture',
    whyItMatters: 'Importing from internal paths of a package bypasses its public API and couples your code to implementation details that may change without notice.',
  ...
  'boundary-violation': {
    ...
    category: 'architecture',
    whyItMatters: 'Crossing module boundaries creates tight coupling. Features that import from other features or layers break isolation and make independent deployment and testing impossible.',
  ...
  'public-api-violation': {
    ...
    category: 'architecture',
    whyItMatters: 'Importing internal files of a module instead of its barrel file (index.ts) couples consumers to implementation details. When the module refactors internals, all consumers break.',
  ...
  'large-component': {
    ...
    category: 'performance',
    whyItMatters: 'Large components are hard to test, reason about, and maintain. They often indicate mixed concerns that should be split into child components.',
  ...
  'eager-route-component': {
    ...
    category: 'performance',
    whyItMatters: 'Eagerly loaded route components increase the initial bundle size. Lazy loading defers the load until the user navigates to that route.',
  ...
  'eager-route-children': {
    ...
    category: 'performance',
    whyItMatters: 'Inlined children arrays prevent code splitting. Using loadChildren keeps sub-routes in a separate chunk.',
  ...
  'circular-service-injection': {
    ...
    category: 'architecture',
    whyItMatters: 'Circular injection causes runtime errors or requires forwardRef hacks. It indicates tangled business logic that should be restructured.',
  ...
  'forward-ref-usage': {
    ...
    category: 'architecture',
    whyItMatters: 'forwardRef is a workaround for circular dependencies. Its presence usually signals an architecture issue that should be resolved.',
  ...
  '@angular-eslint/template/banana-in-box': {
    ...
    category: 'best-practices',
    whyItMatters: 'Writing `([ngModel])` instead of `[(ngModel)]` silently creates a one-way binding instead of two-way, causing form values not to update.',
  ...
  'rxjs-x/no-unsafe-takeuntil': {
    ...
    category: 'best-practices',
    whyItMatters: 'Placing takeUntil before operators like switchMap or mergeMap creates inner subscriptions that are not cleaned up when the notifier fires, causing memory leaks.',
  ...
  'rxjs-x/no-ignored-subscription': {
    ...
    category: 'best-practices',
    whyItMatters: 'Calling .subscribe() without storing or managing the returned Subscription means you have no way to unsubscribe, leading to memory leaks and stale callbacks.',
  ...
  'bypass-security-trust': {
    ...
    category: 'security',
    whyItMatters: 'Calling bypassSecurityTrust* disables Angular\'s built-in XSS protection for that value. If the input is ever user-controlled, it opens the door to cross-site scripting attacks.',
  ...
  'eval-usage': {
    ...
    category: 'security',
    whyItMatters: 'eval() and new Function() execute arbitrary strings as code, enabling code injection attacks. They also prevent JavaScript engine optimizations.',
  ...
  'hardcoded-secret': {
    ...
    category: 'security',
    whyItMatters: 'Hardcoded secrets in source code get committed to version control, appear in build artifacts, and can be extracted by anyone with repository access.',
  ...
  'innerhtml-binding': {
    ...
    category: 'security',
    whyItMatters: 'Binding to [innerHTML] can render unsanitized HTML. While Angular sanitizes by default, dynamic HTML from user input or APIs can still introduce XSS risks.',
  ...
  'low-signal-readiness': {
    ...
    category: 'best-practices',
    whyItMatters: 'Angular Signals provide fine-grained reactivity and eliminate the need for manual subscription management. Adopting signals improves performance and reduces boilerplate.',
