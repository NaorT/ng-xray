...
const priorityEmoji = (p: RemediationItem["priority"]): string => (p === "high" ? "🔴" : p === "medium" ? "🟡" : "🟢");
...
export const generatePrSummary = (result: ScanResult): string => {
  ...
  if (result.scanStatus === "partial" && result.failedAnalyzers.length > 0) {
    lines.push(
      `> ⚠️ Partial scan — ${result.failedAnalyzers.length} analyzer(s) failed: ${result.failedAnalyzers.join(", ")}`,
    );
  ...
  if (topFixes.length > 0) {
    ...
    for (const r of topFixes) {
      const impact = r.includedInScore === false ? "advisory" : `+${r.estimatedScoreImpact} pts`;
      lines.push(
        `- ${priorityEmoji(r.priority)} ${escapeMarkdown(r.description)} — ${impact}, ${r.affectedFileCount} file${r.affectedFileCount === 1 ? "" : "s"}`,
      );
    }
    lines.push("");
  }
