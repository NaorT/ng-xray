...
const RULE_HELP_MAP: Record<string, string> = {
  ...
  "@angular-eslint/use-lifecycle-interface":
    "Implement the corresponding lifecycle interface (e.g., `implements OnInit`).",
  "@angular-eslint/no-empty-lifecycle-method": "Remove empty lifecycle methods or add the required logic.",
  "@angular-eslint/contextual-lifecycle":
    "Lifecycle hooks can only be used in classes decorated with @Component, @Directive, or @Pipe.",
  ...
  "@angular-eslint/template/use-track-by-function":
    "Add a `trackBy` function to *ngFor or a `track` expression to @for to avoid unnecessary DOM re-renders.",
  "@angular-eslint/template/no-duplicate-attributes":
    "Remove duplicate attributes from the element. Duplicate attributes cause unpredictable behavior.",
  "@angular-eslint/template/banana-in-box":
    "Fix two-way binding syntax: use `[(ngModel)]` (banana-in-box), not `([ngModel])`.",
  "@angular-eslint/template/conditional-complexity":
    "Simplify the template condition — extract complex logic into a component property or pipe.",
  "@angular-eslint/template/cyclomatic-complexity":
    "Reduce template branching complexity by extracting sections into child components.",
  "@angular-eslint/template/alt-text": "Add an `alt` attribute to <img> and other media elements for accessibility.",
  "@angular-eslint/template/click-events-have-key-events":
    "Add a keyboard event handler alongside (click) for accessibility.",
  "@angular-eslint/template/elements-content":
    "Ensure interactive elements have meaningful text content or aria-label.",
  "rxjs-x/no-unsafe-takeuntil":
    "Place takeUntil/takeUntilDestroyed as the last operator before subscribe to prevent subscription leaks.",
  "rxjs-x/no-ignored-subscription":
    "Store the Subscription or use takeUntilDestroyed/async pipe to manage the subscription lifecycle.",
};
...
export const runLintAnalyzer = async (directory: string): Promise<Diagnostic[]> => {
  ...
  const files = [
    ...
    ...walkFiles(targetDir, [".html"], { skipDeclarations: false }).map((filePath) =>
  ...
  try {
    ...
    throw new Error(`ESLint failed (${mode} mode): ${msg}. Run with --no-lint to skip lint checks.`);
  }
};
