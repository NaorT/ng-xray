...
const findUp = (directory: string, resolveCandidate: (current: string) => string | null): string | null => {
...
export const resolveAngularCompilerContext = (directory: string): { ngcBinary: string; tsConfig: string } | null => {
...
export const parseDiagnosticOutput = (output: string): ParsedDiagnostic[] => {
...
export const runAngularDiagnosticsAnalyzer = async (
  ...
  let output: string;
  try {
    const result = await execFileAsync(ngcBinary, ["-p", tsConfig, "--noEmit"], {
      cwd: directory,
      encoding: "utf-8",
      timeout: 120_000,
      signal,
    ...
    if (!output.includes("NG")) {
      throw new Error(`Angular compiler failed with exit code ${execError.code}: ${output.slice(0, 500)}`);
    }
  }
  ...
  for (const d of parsed) {
    const relPath = d.filePath.startsWith("/") ? path.relative(directory, d.filePath) : d.filePath;
