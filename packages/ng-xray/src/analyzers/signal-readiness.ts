...
const EFFORT_MAP: Record<string, { effort: 'low' | 'medium' | 'high'; description: string }> = {
  ...
  'ViewChild': { effort: 'medium', description: 'Migrate @ViewChild() to viewChild() — check timing in lifecycle hooks.' },
  'ContentChild': { effort: 'medium', description: 'Migrate @ContentChild() to contentChild() — check projected content lifecycle.' },
  'Injection': { effort: 'low', description: 'Migrate constructor injection to inject() — mechanical, no behavior change.' },
  'State': { effort: 'high', description: 'Migrate BehaviorSubject/ReplaySubject to signal() — requires understanding data flow.' },
  'Subscriptions': { effort: 'high', description: 'Replace .subscribe() with toSignal()/effect() — requires async flow analysis.' },
};
...
export const analyzeSignalReadiness = (directory: string): SignalReadinessReport => {
  ...
  for (const filePath of files) {
    try {
      ...
      logger.debug(`Signal readiness: failed to read ${filePath} — ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
export const runSignalReadinessAnalyzer = async (directory: string): Promise<{ diagnostics: Diagnostic[]; report: SignalReadinessReport }> => {
  ...
  if (report.score < 50) {
    diagnostics.push({
      ...
      help: report.migrationPlan.length > 0
