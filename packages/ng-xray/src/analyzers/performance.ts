...
const checkMissingOnPush = (filePath: string, sourceFile: ReturnType<Project['createSourceFile']>): Diagnostic | null => {
...
const checkHeavyConstructor = (filePath: string, sourceFile: ReturnType<Project['createSourceFile']>): Diagnostic | null => {
  for (const classDecl of sourceFile.getClasses()) {
...
const checkBarrelBloat = (filePath: string, content: string): Diagnostic | null => {
  if (!filePath.endsWith('index.ts')) return null;

  const exportLines = content.split('\n').filter((line) => line.trim().startsWith('export'));
  if (exportLines.length <= 10) return null;

  const hasWildcardExports = exportLines.some((line) => line.includes('export *'));
  if (!hasWildcardExports) return null;
  ...
  return {
    ...
    category: 'performance',
    severity: 'warning',
    message: `Barrel file with ${exportLines.length} export statements including wildcard re-exports. This can prevent tree-shaking.`,
...
export const runPerformanceAnalyzer = async (
  ...
  for (const filePath of tsFiles) {
    try {
      ...
      const barrel = checkBarrelBloat(relPath, content);
      if (barrel) diagnostics.push(barrel);
    } catch (error) {
      logger.error(`Performance analyzer: failed to read ${filePath} — ${error instanceof Error ? error.message : String(error)}`);
    }
  }
