...
const resolveImportPath = (directory: string, filePath: string, specifier: string): string => {
...
const isAllowedBarrel = (resolvedPath: string, barrelFiles: string[]): boolean => {
  ...
  if (!path.extname(resolvedPath) && barrelFiles.some((b) => path.basename(b, path.extname(b)) === basename))
    return true;
  return false;
};
        ...
        for (const { fromMatch, disallowMatch, rule } of boundaryMatchers) {
          ...
          if (isDisallowed) {
            diagnostics.push({
              ...
              message:
                rule.message ?? `Boundary violation: ${relPath} imports from ${resolvedPath} which is disallowed`,
        ...
        if (specifier.startsWith(".") || specifier.startsWith("/")) {
          for (const { zoneMatch, allowed, rule } of publicApiMatchers) {
            ...
            diagnostics.push({
              ...
              message:
                rule.message ?? `Import bypasses public API: ${resolvedPath}. Import from the barrel file instead.`,
