...
const checkBypassSecurityTrust = (relPath: string, content: string, morphProject: Project): Diagnostic[] => {
...
const checkEvalUsage = (relPath: string, content: string, morphProject: Project): Diagnostic[] => {
  const diagnostics: Diagnostic[] = [];
  const sourceFile = morphProject.createSourceFile(relPath, content, { overwrite: true });
...
const isPlaceholderFile = (filePath: string): boolean => {
  ...
  return (
    base.includes(".mock.") || base.includes(".stub.") || base.includes(".fake.") || base.startsWith("environment")
  );
};
...
const looksLikePlaceholder = (value: string): boolean => {
  const stripped = value
    ...
    .trim()
    .toLowerCase();
  if (PLACEHOLDER_VALUES.has(stripped)) return true;
  if (PLACEHOLDER_PREFIXES.some((p) => stripped.startsWith(p.toLowerCase()))) return true;
...
const checkHardcodedSecrets = (relPath: string, content: string, morphProject: Project): Diagnostic[] => {
  const diagnostics: Diagnostic[] = [];
  const sourceFile = morphProject.createSourceFile(relPath, content, { overwrite: true });
  ...
  const catchAllPattern = SECRET_PATTERNS[SECRET_PATTERNS.length - 1];

  for (const node of stringNodes) {
    const text = node.getText();
    for (const { pattern, label } of SECRET_PATTERNS) {
      if (pattern.test(text)) {
        if (pattern === catchAllPattern.pattern && looksLikePlaceholder(text)) {
          break;
        }
        diagnostics.push({
          filePath: relPath,
...
const visitTemplateNodes = (nodes: TmplAstNode[], visitor: (node: TmplAstNode) => void): void => {
...
export const runSecurityAnalyzer = async (directory: string, prebuiltMorphProject?: Project): Promise<Diagnostic[]> => {
  ...
  for (const filePath of tsFiles) {
    try {
      ...
      logger.error(
        `Security analyzer: failed to read ${filePath} — ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
