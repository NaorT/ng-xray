...
describe("calculateScore", () => {
  ...
  it("category deduction is capped at maxDeduction", () => {
    const diags = Array.from({ length: 30 }, () => makeDiag({ category: "best-practices", severity: "warning" }));
  ...
  it("mixed severity within one category", () => {
    const diags: Diagnostic[] = [
      ...Array(2)
        .fill(null)
        .map(() => makeDiag({ category: "architecture", severity: "error", rule: "e1" })),
      ...Array(3)
        .fill(null)
        .map(() => makeDiag({ category: "architecture", severity: "warning", rule: "w1" })),
    ];
    const result = calculateScore(diags);
  ...
  it("score never drops below 0", () => {
    ...
    for (const cat of categories) {
      for (let j = 0; j < 100; j++) {
        diags.push(makeDiag({ category: cat, severity: "error", rule: `spam-${cat}-${j}` }));
      }
    }
    const result = calculateScore(diags);
  ...
  it("always returns all 5 categories, even with no diagnostics", () => {
    const result = calculateScore([]);
    const expectedCategories: Category[] = ["best-practices", "performance", "architecture", "dead-code", "security"];
    expect(result.categories).toHaveLength(5);
    for (const cat of expectedCategories) {
      expect(result.categories.find((c) => c.category === cat)).toBeDefined();
    }
  });

  it("density multiplier reduces deduction for low-density projects", () => {
    const diags = Array.from({ length: 5 }, (_, i) =>
      makeDiag({ category: "best-practices", severity: "warning", rule: `rule-${i}` }),
    );
    const noFile = calculateScore(diags);
    const sparse = calculateScore(diags, { fileCount: 100 });
    expect(sparse.overall).toBeGreaterThan(noFile.overall);
  });
});
...
describe("generateRemediation", () => {
  ...
  it("priority is high when impact >= 6, medium when >= 3, low otherwise", () => {
    const highDiags = Array(3)
      .fill(null)
      .map(() => makeDiag({ rule: "high", category: "best-practices", severity: "error" }));
  ...
  it("applies density scaling to remediation impact when fileCount is provided", () => {
    const diags = [makeDiag({ rule: "missing-onpush", category: "performance", severity: "error" })];
